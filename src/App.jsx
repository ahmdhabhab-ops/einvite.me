...
function DashboardView({ guestGroups, addGuestGroup, updateGuestGroup, deleteGuestGroup, moveGuestGroup, tables, addTable, updateTable, deleteTable, assignGuestToTable, integrations, coupleTitle, slug }) {
  ...
  const openLink = `https://einvite.me/e/${slug}`;
  ...
  const guestLink = (group) => `https://einvite.me/e/${slug}?g=${group.id}`;
...
function UsersView({ users, invitationsStore, onDelete, onToggleStatus, onCreateInvitationFor, onApprove, onToggleDashboardAccess, onToggleCanDesign }) {
  const [search, setSearch] = useState("");
  ...
  const copyUserLink = async (user) => {
    const link = `https://einvite.me/e/${user.invitationSlug}`;
    const ok = await copyToClipboard(link);
          ...
          return (
            <div key={u.id} className="grid grid-cols-1 gap-2 border-b px-5 py-2.5 lg:grid-cols-[1.1fr_1.4fr_0.6fr_0.8fr_0.7fr_1.7fr] lg:items-center lg:gap-3" style={{ borderColor: "rgba(147,166,155,0.1)" }}>
              ...
              <div className="flex min-w-0 items-center gap-1.5">
                ...
                {u.invitationSlug ? (
                  <>
                    <a href={`https://einvite.me/e/${u.invitationSlug}`} target="_blank" rel="noreferrer" title={`Open invitation: einvite.me/e/${u.invitationSlug}`} style={{ color: GOLD_SOFT, flexShrink: 0 }}>
                      <Link2 size={12} />
                    </a>
...
export default function InvitationBuilder() {
  ...
  const saveDraft = async () => {
    ...
    const imageJobs = [
      ...
      ...ALL_STEPS.map(({ key }) => window.storage.set(bgKey(key), JSON.stringify(pageBackgrounds[key]), false)),
      ...LANGS.filter((lang) => intro.media[lang]?.type === "image").map((lang) => window.storage.set(introBgKey(lang), intro.media[lang].url, false)),
    ];
  ...
  <GoldButton onClick={async () => {
    ...
    <Check size={14} /> Finish & View Dashboard
        ...
        {view === "settings" && (
          <>
            <SettingsView og={og} setOg={setOg} autoTitle={autoTitle} autoDescription={autoDescription} slug={slug} />
