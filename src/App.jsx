...
import {
  ...
  ChevronsUp, Volume2, VolumeX, Share2,
} from "lucide-react";
...
const defaultIntroSettings = {
  ...
  videoUrl: null,
  videoName: "",
  bgImages: { en: null, ar: null, fr: null, es: null },
};

const FONT_OPTIONS = [
  { key: "auto", label: "Default", value: null },
  { key: "display", label: "Display serif (Fraunces)", value: "'Fraunces', serif" },
  { key: "script", label: "Script (Parisienne)", value: "'Parisienne', cursive" },
  { key: "body", label: "Sans (Inter)", value: "'Inter', sans-serif" },
  { key: "arabic", label: "Arabic (Cairo)", value: "'Cairo', sans-serif" },
];
const fontValue = (key) => FONT_OPTIONS.find((f) => f.key === key)?.value || null;

const emptyCustomBlocks = () => ({ cover: [], family: [], timeline: [], locations: [], rsvp: [] });
...
const STEPS = [
  ...
  { key: "rsvp", label: "Countdown & RSVP", icon: CalendarClock },
];
...
const PREVIEW_T = {
  en: { orderOfDay: "Order of the day", celebration: "The Celebration", countingDownTo: "Counting down to", celebrationWord: "the celebration", celebrationBegun: "The celebration has begun!", days: "days", hrs: "hrs", min: "min", sec: "sec", swipeUp: "Swipe up", directions: "Get Directions", tapToStart: "Tap to start" },
  ar: { orderOfDay: "برنامج اليوم", celebration: "مراسم الاحتفال", countingDownTo: "العد التنازلي لـ", celebrationWord: "الاحتفال", celebrationBegun: "لقد بدأ الاحتفال!", days: "يوم", hrs: "ساعة", min: "دقيقة", sec: "ثانية", swipeUp: "اسحب لأعلى", directions: "احصل على الاتجاهات", tapToStart: "اضغط للبدء" },
  fr: { orderOfDay: "Déroulé de la journée", celebration: "La Célébration", countingDownTo: "Compte à rebours vers", celebrationWord: "la célébration", celebrationBegun: "La célébration a commencé !", days: "jours", hrs: "h", min: "min", sec: "s", swipeUp: "Glissez vers le haut", directions: "Itinéraire", tapToStart: "Touchez pour commencer" },
  es: { orderOfDay: "Orden del día", celebration: "La Celebración", countingDownTo: "Cuenta atrás para", celebrationWord: "la celebración", celebrationBegun: "¡La celebración ha comenzado!", days: "días", hrs: "h", min: "min", sec: "s", swipeUp: "Desliza hacia arriba", directions: "Cómo llegar", tapToStart: "Toca para comenzar" },
};
...
const defaultPageBackgrounds = {
  cover: { mode: "photo", preset: "botanical", image: null },
  family: { mode: "paper", preset: "blush", image: null },
  timeline: { mode: "paper", preset: "dusk", image: null },
  locations: { mode: "paper", preset: "gilded", image: null },
  rsvp: { mode: "paper", preset: "botanical", image: null },
};
...
const DEFAULT_LAYOUTS = {
  ...
  rsvp: { heading: { x: 50, y: 17 }, countdown: { x: 50, y: 44 }, buttons: { x: 50, y: 81 } },
};
...
const seedGuests = () => {
  ...
  const day = 86400000;
  return [
    { id: uid(), name: "Sarah Malik", status: "yes", lang: "en", respondedAt: now - 2 * day },
    { id: uid(), name: "Ahmed Nasser", status: "yes", lang: "ar", respondedAt: now - 1 * day },
    { id: uid(), name: "Julie Laurent", status: "no", lang: "fr", respondedAt: now - 3 * day },
    { id: uid(), name: "Carlos Mendes", status: "yes", lang: "es", respondedAt: now - 4 * day },
    { id: uid(), name: "Fatima Zahra", status: "pending", lang: "ar", respondedAt: null },
    { id: uid(), name: "Tom Whitfield", status: "yes", lang: "en", respondedAt: now - 5 * day },
    { id: uid(), name: "Camille Rousseau", status: "pending", lang: "fr", respondedAt: null },
    { id: uid(), name: "Lucía Fernández", status: "yes", lang: "es", respondedAt: now - 6 * day },
    { id: uid(), name: "Omar Haddad", status: "no", lang: "ar", respondedAt: now - 2 * day },
    { id: uid(), name: "Grace Kim", status: "yes", lang: "en", respondedAt: now - 1 * day },
    { id: uid(), name: "Pierre Dubois", status: "pending", lang: "fr", respondedAt: null },
    { id: uid(), name: "Mateo García", status: "no", lang: "es", respondedAt: now - 7 * day },
  ];
};
...
function LangSwitcher({ activeLang, setActiveLang, defaultLang, setDefaultLang }) {
  return (
    <div className="mb-6 flex flex-wrap items-center gap-2 rounded-xl p-3" style={{ background: INK_3 }}>
      ...
      <span className="ml-auto text-[10.5px]" style={{ color: MUTED, fontFamily: FONT_BODY }}>
        Tap <Star size={9} style={{ display: "inline", verticalAlign: "middle" }} /> to set the default language guests see first
      </span>
    </div>
  );
}
    ...
    <div className="mb-5 rounded-xl p-4" style={{ background: INK_3, border: `1px solid rgba(201,164,76,0.3)` }}>
      <div className="mb-3 flex items-center justify-between">
        <span className="text-[11px] font-semibold uppercase" style={{ color: GOLD_SOFT, letterSpacing: "0.1em", fontFamily: FONT_BODY }}>
          {isCustom ? "Custom text" : "Text style"}
        </span>
      ...
      <div className="mt-4 flex items-center gap-2">
        <GhostButton onClick={() => onChangeStyle({ fontFamily: null, color: null, fontSize: null })}>Reset style</GhostButton>
        {isCustom && (
...
function StepRail({ activeIndex, visited, onSelect }) {
  return (
      ...
      {STEPS.map((step, i) => {
        ...
        return (
          <React.Fragment key={step.key}>
            ...
            {i < STEPS.length - 1 && <div className="mt-[19px] h-px flex-1" style={{ background: isVisited ? GOLD : "rgba(147,166,155,0.25)", opacity: 0.5 }} />}
          </React.Fragment>
        );
      })}
    </div>
...
function CoverStep({ c, updateContent, bg, setBg, music, updateMusic, onUploadAudio, onRemoveAudio, intro, updateIntro, activeLang, onUploadIntroImage, onRemoveIntroImage, onUploadIntroVideo, onRemoveIntroVideo }) {
  return (
    <div>
      <div className="grid grid-cols-2 gap-3">
        <div>
      ...
      <Select
        ...
        options={[{ value: "button", label: "Tap to start button" }, { value: "animation", label: "Animation" }, { value: "video", label: "Video" }]}
      />
      ...
      {intro.type === "video" && (
        <div className="mt-3">
          ...
          <div className="flex items-center gap-2">
            <GhostUploadButton accept="video/*" onChange={onUploadIntroVideo}>
              <Film size={13} /> {intro.videoName ? "Replace video" : "Upload video"}
            </GhostUploadButton>
            {intro.videoName && (
              <span className="flex items-center gap-1 text-xs" style={{ color: MUTED, fontFamily: FONT_BODY }}>
                <Film size={12} /> {intro.videoName}
                <button onClick={onRemoveIntroVideo} className="ml-1" style={{ color: "#E29B9B" }}>
                  <X size={12} />
                </button>
              </span>
            )}
          ...
          <p className="mt-1.5 text-[10.5px]" style={{ color: MUTED, fontFamily: FONT_BODY }}>
            A looping muted video plays full-screen behind the tap-to-start text.
          </p>
        </div>
      )}
      ...
      <div className="mt-4">
        <FieldLabel>Intro background image ({LANG_META[activeLang].short})</FieldLabel>
        <div className="flex items-center gap-3">
          <label className="flex h-14 w-14 cursor-pointer items-center justify-center overflow-hidden rounded-lg" style={{ border: intro.bgImages[activeLang] ? `2px solid ${GOLD}` : `2px dashed rgba(147,166,155,0.5)`, background: intro.bgImages[activeLang] ? `url(${intro.bgImages[activeLang]}) center/cover` : "transparent" }}>
            {!intro.bgImages[activeLang] && <Upload size={16} style={{ color: MUTED }} />}
            <input type="file" accept="image/*" style={VISUALLY_HIDDEN} onChange={onUploadIntroImage} />
          </label>
          <div className="text-[10.5px]" style={{ color: MUTED, fontFamily: FONT_BODY }}>
            {intro.bgImages[activeLang] ? (
              <button onClick={onRemoveIntroImage} className="underline">Remove, fall back to cover background</button>
            ) : (
              "No override for this language — using the Cover slide background above."
            )}
          </div>
        </div>
      </div>
...
function RsvpStep({ c, updateContent, schedule, setSchedule, bg, setBg }) {
  return (
    <div>
      <div className="grid grid-cols-2 gap-3">
        <div>
      ...
      <p className="mt-1.5 text-[10.5px]" style={{ color: MUTED, fontFamily: FONT_BODY }}>
        Date and time are shared across every language; each language shown below has its own button wording.
      </p>
      <Divider />
      <div className="grid grid-cols-2 gap-3">
        <div>
...
function DraggableBlock({ id, pos, editMode, onMove, label, light, children, selected, onSelect }) {
...
function CustomTextBlock({ block, light, editMode, selected, onSelect, onMove }) {
  return (
...
function StoryPage({ bg, children }) {
  ...
  return (
    <div className="relative h-full w-full" style={{ background }}>
      {isPhoto && <div className="absolute inset-0" style={{ background: "linear-gradient(180deg, rgba(10,12,10,0.35) 0%, rgba(10,12,10,0.15) 40%, rgba(10,12,10,0.55) 100%)" }} />}
...
function RsvpSlide({ content, schedule, lang, bg, fontDisplay, fontScript, t, locale, layout, editMode, onMoveBlock, selectedBlock, onSelectBlock }) {
  const cd = useCountdown(schedule.date, schedule.time);
  const hs = layout.heading, cs = layout.countdown, bs = layout.buttons;
    ...
    <StoryPage bg={bg}>
      {(light) => (
        <div className="relative h-full w-full">
          ...
          <DraggableBlock id="buttons" pos={bs} editMode={editMode} onMove={(p) => onMoveBlock("buttons", p)} label="RSVP buttons" light={light} selected={selectedBlock === "buttons"} onSelect={() => onSelectBlock("buttons")}>
            <div className="flex flex-col gap-2" style={{ width: 220 }}>
              <button className="rounded-full py-2.5 font-semibold" style={{ background: light ? GOLD : EMERALD, color: bs.color || (light ? INK : PAPER), fontFamily: bs.fontFamily || FONT_BODY, fontSize: bs.fontSize ? `${bs.fontSize}px` : 12 }}>{content.yesLabel}</button>
              <button className="rounded-full py-2.5 font-semibold" style={{ background: "transparent", color: bs.color || (light ? PAPER : ROSE), border: `1.5px solid ${light ? PAPER : ROSE}`, fontFamily: bs.fontFamily || FONT_BODY, fontSize: bs.fontSize ? `${bs.fontSize}px` : 12 }}>{content.noLabel}</button>
            </div>
          </DraggableBlock>
        </div>
      )}
...
function PhonePreview({ data, activeIndex, onNavigate, lang, layoutEditMode, onMoveBlock, started, onStart, selectedBlockId, onSelectBlock, onMoveCustomBlock }) {
  ...
  const gateImage = data.intro.bgImages[lang] || data.pageBackgrounds.cover.image;
  ...
  if (data.music.enabled) {
    bottomRightActions.push({ key: "music", icon: playing ? Volume2 : VolumeX, onClick: () => setPlaying((p) => !p) });
  }
  ...
  const slides = [
    <CoverSlide content={data.content[lang].cover} bg={data.pageBackgrounds.cover} fontDisplay={fontDisplay} fontScript={fontScript} layout={data.layouts.cover} editMode={layoutEditMode} onMoveBlock={(id, p) => onMoveBlock("cover", id, p)} selectedBlock={selectedBlockId} onSelectBlock={onSelectBlock} />,
    <FamilySlide content={data.content[lang].family} bg={data.pageBackgrounds.family} fontDisplay={fontDisplay} layout={data.layouts.family} editMode={layoutEditMode} onMoveBlock={(id, p) => onMoveBlock("family", id, p)} selectedBlock={selectedBlockId} onSelectBlock={onSelectBlock} />,
    <TimelineSlide items={data.timeline} lang={lang} bg={data.pageBackgrounds.timeline} fontDisplay={fontDisplay} t={t} layout={data.layouts.timeline} editMode={layoutEditMode} onMoveBlock={(id, p) => onMoveBlock("timeline", id, p)} selectedBlock={selectedBlockId} onSelectBlock={onSelectBlock} />,
    <LocationsSlide items={data.locations} lang={lang} bg={data.pageBackgrounds.locations} fontDisplay={fontDisplay} t={t} layout={data.layouts.locations} editMode={layoutEditMode} onMoveBlock={(id, p) => onMoveBlock("locations", id, p)} selectedBlock={selectedBlockId} onSelectBlock={onSelectBlock} />,
    <RsvpSlide content={data.content[lang].rsvp} schedule={data.rsvpSchedule} lang={lang} bg={data.pageBackgrounds.rsvp} fontDisplay={fontDisplay} fontScript={fontScript} t={t} locale={LANG_META[lang].locale} layout={data.layouts.rsvp} editMode={layoutEditMode} onMoveBlock={(id, p) => onMoveBlock("rsvp", id, p)} selectedBlock={selectedBlockId} onSelectBlock={onSelectBlock} />,
  ];

  return (
    <div className="flex flex-col items-center">
      <div className="relative" style={{ width: 292, height: 600, background: "#000", borderRadius: 42, padding: 10, boxShadow: "0 30px 60px -20px rgba(0,0,0,0.6), 0 0 0 1px rgba(201,164,76,0.15)" }}>
        <div className="relative h-full w-full overflow-hidden" style={{ borderRadius: 32, background: PAPER }} dir={dir} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd} onWheel={onWheel}>
          ...
          {!started && (
            <button onClick={onStart} className="absolute inset-0 z-40 flex flex-col items-center justify-center gap-4 overflow-hidden" style={{ background: gateBackground, cursor: "pointer" }}>
              {data.intro.type === "video" && data.intro.videoUrl && (
                <video src={data.intro.videoUrl} autoPlay muted loop playsInline className="absolute inset-0 h-full w-full object-cover" />
              )}
              ...
              <div className="absolute bottom-5 right-3 z-20 flex flex-col items-center gap-2">
                {bottomRightActions.map((action) => (
                  <button
                    ...
                    style={{ background: "rgba(10,12,10,0.5)", backdropFilter: "blur(4px)" }}
                  >
                    <action.icon size={14} color={PAPER} />
                  </button>
                ))}
              </div>
            </>
...
function SettingsView({ og, setOg, autoTitle, autoDescription, slug }) {
  ...
  const link = `https://einvite.me/e/${slug}`;
  ...
  return (
    <div className="mx-auto max-w-2xl rounded-2xl p-6" style={{ background: INK_2, border: `1px solid rgba(201,164,76,0.12)` }}>
      ...
      <div className="mt-6">
        ...
        <WhatsAppPreviewCard image={og.image} title={og.title || autoTitle} description={og.description || autoDescription} domain="einvite.me" />
      </div>
    </div>
  );
}
...
function DashboardView({ guests, addGuest, coupleTitle }) {
  ...
  const submit = () => {
    if (!name.trim()) return;
    addGuest({ id: uid(), name: name.trim(), status, lang: "en", respondedAt: Date.now() });
    setName("");
  };

  return (
    <div className="mx-auto max-w-4xl">
      <h2 className="mb-1 text-lg" style={{ fontFamily: FONT_DISPLAY, fontStyle: "italic", color: IVORY }}>RSVP dashboard</h2>
      <p className="mb-6 text-[12px]" style={{ color: MUTED, fontFamily: FONT_BODY }}>Live responses for {coupleTitle}</p>
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 rounded-2xl p-5 sm:grid-cols-2" style={{ background: INK_2, border: `1px solid rgba(201,164,76,0.12)` }}>
        ...
        <div className="flex flex-col justify-center gap-3">
          <FieldLabel>Log a response</FieldLabel>
          <TextInput value={name} onChange={setName} placeholder="Guest name" />
          <div className="flex gap-2">
            <GhostButton active={status === "yes"} onClick={() => setStatus("yes")}><CheckCircle2 size={13} /> Attending</GhostButton>
            <GhostButton active={status === "no"} onClick={() => setStatus("no")}><XCircle size={13} /> Not attending</GhostButton>
          </div>
          <GoldButton onClick={submit}><Plus size={14} /> Add response</GoldButton>
        </div>
      </div>

      <div className="rounded-2xl p-5" style={{ background: INK_2, border: `1px solid rgba(201,164,76,0.12)` }}>
        <div className="mb-4 flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-2 rounded-lg px-3 py-2" style={{ background: INK_3, flex: 1, minWidth: 160 }}>
            <Search size={13} color={MUTED} />
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search guests" className="w-full bg-transparent text-sm outline-none" style={{ color: IVORY, fontFamily: FONT_BODY }} />
          </div>
        ...
        <div className="space-y-1.5">
          {filtered.map((g) => (
            <div key={g.id} className="flex items-center justify-between rounded-lg px-3 py-2.5" style={{ background: INK_3 }}>
              <div className="flex items-center gap-2.5">
                {g.status === "yes" && <CheckCircle2 size={15} color={CHART_COLORS.yes} />}
                {g.status === "no" && <XCircle size={15} color={CHART_COLORS.no} />}
                {g.status === "pending" && <Clock size={15} color={CHART_COLORS.pending} />}
                <span className="text-[13px]" style={{ color: IVORY, fontFamily: FONT_BODY }}>{g.name}</span>
              </div>
              <div className="flex items-center gap-3">
                <span className="rounded-full px-2 py-0.5 text-[9.5px] font-semibold uppercase" style={{ background: INK_2, color: MUTED, fontFamily: FONT_BODY }}>{LANG_META[g.lang]?.short}</span>
...
export default function InvitationBuilder() {
  ...
  const [music, setMusic] = useState({ enabled: true, url: null, name: "" });
  ...
  const resetLayout = () => {
    setLayouts((l) => ({ ...l, [STEPS[activeIndex].key]: { ...DEFAULT_LAYOUTS[STEPS[activeIndex].key] } }));
    setCustomBlocks((c) => ({ ...c, [STEPS[activeIndex].key]: [] }));
  ...
  useEffect(() => {
    if (typeof window === "undefined" || !window.storage) return; // storage API not present in this environment
    let cancelled = false;
    (async () => {
      try {
        ...
        if (d.customBlocks) setCustomBlocks(d.customBlocks);
    ...
    STEPS.forEach(({ key }) => {
      (async () => {
        try {
          const res = await window.storage.get(bgKey(key), false);
          if (cancelled || !res?.value) return;
    ...
    LANGS.forEach((lang) => {
      (async () => {
        try {
          const res = await window.storage.get(introBgKey(lang), false);
          if (cancelled || !res?.value) return;
          setIntro((i) => ({ ...i, bgImages: { ...i.bgImages, [lang]: res.value } }));
        } catch {}
      })();
    });
    (async () => {
      try {
        const res = await window.storage.get(OG_IMAGE_KEY, false);
        if (cancelled || !res?.value) return;
  ...
  const saveDraft = async () => {
    ...
    const corePayload = {
      content, timeline, locations, rsvpSchedule, defaultLang, layouts, customBlocks,
      ogText: { title: og.title, description: og.description },
      intro: { type: intro.type, icon: intro.icon, animationStyle: intro.animationStyle }, // videoUrl omitted: blob URLs don't survive reload
      musicMeta: { enabled: music.enabled, name: music.name }, // url omitted for the same reason
    };
    const imageJobs = [
      window.storage.set(DRAFT_KEY, JSON.stringify(corePayload), false),
      ...STEPS.map(({ key }) => window.storage.set(bgKey(key), JSON.stringify(pageBackgrounds[key]), false)),
      ...LANGS.filter((lang) => intro.bgImages[lang]).map((lang) => window.storage.set(introBgKey(lang), intro.bgImages[lang], false)),
    ];
    if (og.image) imageJobs.push(window.storage.set(OG_IMAGE_KEY, og.image, false));
    try {
  ...
  const addCustomText = () => {
    const stepKey = STEPS[activeIndex].key;
    const newBlock = { id: uid(), text: "New text", x: 50, y: 50, fontFamily: null, color: null, fontSize: 16 };
  ...
  const addGuest = (g) => setGuests((list) => [g, ...list]);
  ...
  const handleIntroVideoUpload = (e) => {
    ...
    if (!file) return;
    const url = URL.createObjectURL(file);
    setIntro((i) => ({ ...i, videoUrl: url, videoName: file.name }));
  };
  const removeIntroVideo = () => setIntro((i) => ({ ...i, videoUrl: null, videoName: "" }));
  const handleIntroImageUpload = async (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    try {
      const dataUrl = await readImageCompressed(file);
      setIntro((i) => ({ ...i, bgImages: { ...i.bgImages, [activeLang]: dataUrl } }));
    } catch {
      const reader = new FileReader();
      reader.onload = () => setIntro((i) => ({ ...i, bgImages: { ...i.bgImages, [activeLang]: reader.result } }));
      reader.readAsDataURL(file);
    }
  };
  const removeIntroImage = () => setIntro((i) => ({ ...i, bgImages: { ...i.bgImages, [activeLang]: null } }));

  const data = { content, timeline, locations, pageBackgrounds, music, rsvpSchedule, layouts, intro, customBlocks };
  const stepKey = STEPS[activeIndex].key;
  ...
  return (
    <div className="min-h-screen w-full" style={{ background: INK, fontFamily: FONT_BODY }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Fraunces:ital,wght@0,400;0,600;1,500&family=Inter:wght@400;500;600;700&family=Parisienne&family=Cairo:wght@400;500;600;700&display=swap');
      ...
      <div className="mx-auto max-w-6xl px-6 py-10">
        <div className="mb-6 flex items-baseline justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {saveStatus === "unavailable" && <span className="text-[11px]" style={{ color: "#E29B9B", fontFamily: FONT_BODY }}>Saving isn't available in this environment</span>}
        ...
        {view === "builder" && (
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_320px]">
            <div className="rounded-2xl p-6" style={{ background: INK_2, border: `1px solid rgba(201,164,76,0.12)` }}>
              <LangSwitcher activeLang={activeLang} setActiveLang={setActiveLang} defaultLang={defaultLang} setDefaultLang={setDefaultLang} />
              <StepRail activeIndex={activeIndex} visited={visited} onSelect={selectStep} />
              ...
              {stepKey === "cover" && (
                <CoverStep
                  ...
                  onUploadIntroImage={handleIntroImageUpload}
                  onRemoveIntroImage={removeIntroImage}
                  onUploadIntroVideo={handleIntroVideoUpload}
                  onRemoveIntroVideo={removeIntroVideo}
                />
              )}
              ...
              {stepKey === "rsvp" && <RsvpStep c={c.rsvp} updateContent={(p) => updateContentSection("rsvp", p)} schedule={rsvpSchedule} setSchedule={(p) => setRsvpSchedule((s) => ({ ...s, ...p }))} bg={pageBackgrounds.rsvp} setBg={setBgFor("rsvp")} />}
            ...
            <div className="flex flex-col items-center gap-3 lg:sticky lg:top-10 lg:self-start">
              <PhonePreview data={data} activeIndex={activeIndex} onNavigate={selectStep} lang={activeLang} layoutEditMode={layoutEditMode} onMoveBlock={moveBlock} started={started} onStart={() => setStarted(true)} />
        ...
        {view === "settings" && <SettingsView og={og} setOg={setOg} autoTitle={autoTitle} autoDescription={autoDescription} slug={slug} />}

        {view === "dashboard" && <DashboardView guests={guests} addGuest={addGuest} coupleTitle={`${c.cover.name1} & ${c.cover.name2}`} />}
      </div>
    </div>
  );
}];
